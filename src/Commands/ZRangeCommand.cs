...
public class ZRangeCommand(Database db) : ICommand
{
    public const string Name = "ZRANGE";
    ...
    public Task<RespObject> Handle(Socket connection, RespObject[] args)
    {
        ...
        var count = sortedSetRecord.Count;
        ...
        var entries = sortedSetRecord.GetEntriesInRange(startIndex, endIndex).Select(x => new BulkString(x.Member)).ToArray<RespObject>();
